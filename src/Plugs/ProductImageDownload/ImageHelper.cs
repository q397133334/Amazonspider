...
namespace Amazonspider.ProductImageDownload
{
    public class ImageHelper
    {
        ...
        public static string ImageWatermark(string path, string waterpath, string location)
        {
            ...
            if (kz_name == ".jpg" || kz_name == ".bmp" || kz_name == ".jpeg" || kz_name == ".png" || kz_name == ".tbi")
            {
                DateTime time = DateTime.Now;
                string filename = "" + time.Year.ToString() + time.Month.ToString() + time.Day.ToString() + time.Hour.ToString() + time.Minute.ToString() + time.Second.ToString() + time.Millisecond.ToString() + Guid.NewGuid().ToString();
        ...
        /// 
        /// 
        public static string JoinImage(string path, string waterpath, string jm = "H")
        {
            string kz_name = Path.GetExtension(path);
            if (kz_name == ".jpg" || kz_name == ".bmp" || kz_name == ".jpeg" || kz_name == ".png" || kz_name == ".tbi")
            {
                ...
                try
                {
                    ...
                    Image waterimg = Image.FromFile(waterpath);
                    int width = 0;
                    width = img.Width + waterimg.Width;
                    int height = img.Height > waterimg.Height ? img.Height : waterimg.Height;
                    ...
                    int currentWidth = 0;
                    // 拼图
                    graph.DrawImage(img, currentWidth, 0);
                    //拼接改图后，当前宽度
                    currentWidth += img.Width;
                    graph.DrawImage(waterimg, currentWidth, 0);
                    //Graphics g = Graphics.FromImage(img);
                    //ArrayList loca = GetLocation(location, img, waterimg);
                    //g.DrawImage(waterimg, new Rectangle(int.Parse(loca[0].ToString()), int.Parse(loca[1].ToString()), waterimg.Width, waterimg.Height));
                    //waterimg.Dispose();
                    //g.Dispose();
                    string newpath = Path.GetDirectoryName(r == 0 ? path : waterpath) + "\\" + filename + kz_name;

                    Image img1;
                    //graph.DrawImage(img1, new Rectangle(0, 0, width, height));
                    tableChartImage.Save(newpath);
                    tableChartImage.Dispose();
                    graph.Dispose();
                    waterimg.Dispose();
                    img.Dispose();
                    File.Copy(newpath, r == 0 ? path : waterpath, true);
                    if (File.Exists(newpath))
                    {
                        File.Delete(newpath);
                    }
        ...
        public static void WriteLog(string msg)
        {
            ...
            try
            {
                string path = System.Environment.CurrentDirectory + @"\PATH.txt";
